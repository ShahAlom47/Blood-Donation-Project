...
const MoneyDonationForm = () => {
    ...
    const storeDonationOption = (option) => {
        localStorage.setItem('donationOption', option)
    ...
    return (
        <div className="my-6 bg-gray-500 bg-opacity-10 h-ful p-4">
            ...
            <div className="flex justify-start my-6">
                <button onClick={() => { storeDonationOption('oneTime') }} className={`border border-color-p px-3 ${donationOption === 'oneTime' ? 'bg-color-p text-white' : ''}`}>One Time</button>
                <button onClick={() => { storeDonationOption('monthly') }} className={`border border-color-p px-3 ${donationOption === 'monthly' ? 'bg-color-p text-white' : ''}`}>Monthly</button>
            </div>
